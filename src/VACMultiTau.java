...
import vac.CalculateAutocorrelationMultiTau;
 ...
 * @author Benjamin
 */
public final class VACMultiTau{
	
	/**
	 * This will perform an all in one calculation to get mean square displacement information.
	 * @param args {binary trajectory file, working directory, output description file, number of resamples, dimension, aggregation number}
	...
	public static void main(String[] args) throws IOException{
		...
		if(dstNam == null || srcNam==null || wrkDir==null || numStr==null || dimStr==null || aggStr==null){
			throw new IllegalArgumentException("Usage: java VACMultiTau -src=trajectory -wrk=working_directory -dst=diffusion_summary -num=number_of_resamples -dim=data_dimensionality -agg=aggregation_number [-adc=diffusion_distribution_file] [-vcr=variance_cutoff_ratio]");
		}
		...
		CalculateAutocorrelationMultiTau.dataDim = Integer.parseInt(dimStr);
		CalculateAutocorrelationMultiTau.main(new String[]{"-src="+sourceFileName, "-dst="+workingDirName + File.separatorChar + "Autocor.vac.sdb", "-agg="+aggStr});
