 ...
 *
 * This class provides a skeletal implementation of the <tt>CollectionInt</tt>
 ...
 * <tt>CollectionInt</tt> constructor, as per the recommendation in the
 * <tt>CollectionInt</tt> interface specification.<p>
 *
...
public abstract class AbstractCollectionInt implements CollectionInt {
     ...
     * implicit.)
     */
    protected AbstractCollectionInt() {
    	//Placeholder
    }
     ...
     */
    public abstract IteratorInt iterator();
    ...
    public boolean contains(final int object) {
        final IteratorInt iter = iterator();
        while(iter.hasNext()){
     ...
     *
     *  <pre> {@code
     * ListInt list = new ArrayListInt(size());
     * for (int e : this)
     *     list.add(e);
     * return list.toArray();
     * }</pre>
    ...
    public int[] toArray(final int[] store) {
        // Estimate size of array; be prepared to see more or fewer elements
        final int size = size();
        int[] arr = store.length >= size ? store : new int[size];
        final IteratorInt iter = iterator();
     ...
     */
    private static int[] finishToArray(final int[] toFinish, final IteratorInt iter) {
    	int[] arr = toFinish;
     ...
     * @throws NullPointerException          {@inheritDoc}
     */
    public boolean remove(final int object) {
        final IteratorInt iter = iterator();
        while(iter.hasNext()){
        	if(object==iter.next()){
        		iter.remove();
     ...
     * @see #contains(int)
     */
    public boolean containsAll(final CollectionInt collection) {
    ...
    public String toString() {
        ...
        toBuild.append('[');
        for (;;) {
            final int toAdd = iter.nextPrimitive();
