 ...
 *
 * The <tt>ListInt</tt> interface places additional stipulations, beyond those
 * specified in the <tt>CollectionInt</tt> interface, on the contracts of the
 * <tt>iterator</tt>, <tt>add</tt>, <tt>remove</tt>, <tt>equals</tt>, and
 * <tt>hashCode</tt> methods.  Declarations for other inherited methods are
 * also included here for convenience.<p>
 *
 * The <tt>ListInt</tt> interface provides four
 * methods for positional (indexed)
 * access to list elements.  Lists (like Java arrays) are zero based.  Note
 * that these operations may execute in time proportional to the index value
 * for some implementations (the <tt>LinkedListInt</tt> class, for
 * example). Thus, iterating over the elements in a list is typically
 * preferable to indexing through it if the caller does not know the
 * implementation.<p>
 *
 * The <tt>ListInt</tt> interface provides a special iterator, called a
 * <tt>ListIteratorInt</tt>, that allows
 * element insertion and replacement, and
 * bidirectional access in addition to the normal operations that the
 * <tt>IteratorInt</tt> interface provides.  A method is provided to obtain a
 * list iterator that starts at a specified position in the list.<p>
 *
 * The <tt>ListInt</tt> interface provides two methods
...
public interface ListInt extends CollectionInt {
    // Query Operations
     ...
     */
    boolean addAll(int index, CollectionInt collection);
     ...
     *        list iterator (by a call to {@link ListIteratorInt#next next})
     * @return a list iterator over the elements in this list (in proper
     *         sequence), starting at the specified position in the list
     * @throws IndexOutOfBoundsException if the index is out of range
     *         ({@code index < 0 || index > size()})
     */
    ListIteratorInt listIterator(int index);
     ...
     */
    ListInt subList(int fromIndex, int toIndex);
}
