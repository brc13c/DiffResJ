 ...
 * @author Benjamin
 */
public final class CalculateSquareDisplacementClipRepeat{
	
	public static int dataDim = -1;
	
	/**
	 * This will calculate the square displacement of the particles in a set of trajectory data.
	 * @param args {File to get displacement for, file to write to, temporary file, number of timesteps before reset}
	...
	public static void main(String[] args) throws IOException{
		...
		if(dstNam == null || srcNam==null || wrkNam == null || clrStr == null){
			throw new IllegalArgumentException("Usage: java msd.CalculateSquareDisplacementClipRepeat -src=input_file -dst=output_file -wrk=working_file -clp=clip_length");
		}
		...
		for(long curTime = 0; curTime < numTimesteps; curTime += timestepRepeat){
			...
			for(int timeInd = 0; timeInd < timestepRepeat; timeInd++){
				...
				for(long part = 0; part < numParticles; part++){
					...
					int locInd = partLoc.getType().getFloats().getFloatVariableIndex("POSITION");
					double disp = 0;
					if(timeInd == 0){
						for(int i = 0; i<partLoc.floatVals[locInd].length; i++){
							startPositions.add(partLoc.floatVals[locInd][i].doubleValue());
						}
					}
					else{
						if(dataDim >= 0){
						...
						for(int i = 0; i<partLoc.floatVals[locInd].length; i++){
							double diff = partLoc.floatVals[locInd][i].doubleValue() - startPositions.get(curInd);
							disp += (diff * diff);
							curInd++;
						}
					}
					autoZero.floatVals[valInd][0] = new ArbitraryFloat(disp + prevDisp);
