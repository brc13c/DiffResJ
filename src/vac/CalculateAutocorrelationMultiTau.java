...
/**
 * This will calculate the autocorrelation of the velocities in a set of trajectory data, using the multi-tau method.
 * @author Benjamin
 */
public final class CalculateAutocorrelationMultiTau{
	...
	public static void main(String[] args) throws IOException{
		...
		if(srcNam == null || dstNam == null || aggStr == null){
			throw new IllegalArgumentException("Usage: java vac.CalculateAutocorrelationMultiTau -src=input_trajectory -dst=output_autocorrelation_file -agg=aggregation_number");
		}
...
//							for(int i = 0; i<velDim; i++){
//								avgVelocities[curRead].set(basePtr+i, avgVelocities[curRead].get(basePtr+i) + curStruct.floatVals[velInd][i].doubleValue());
//							}
						}
//						basePtr += velDim;
						curStruct = particleData.readNextEntry();
				...
				for(int j = i; j<aggregationNumber; j++){
					...
					for(int p = 0; p < numDims.size(); p++){
						...
						for(int d = 0; d<velDim; d++){
							double veli = avgVelocities[i].get(basePtr + d); // / curAggregation
							double velj = avgVelocities[j].get(basePtr + d); // / curAggregation
							autocor += (veli * velj);
						}
...
//						basePtr += velDim;
	...
	private CalculateAutocorrelationMultiTau(){
		//force pure static
	}
}