...
import msd.CalculateSquareDisplacementClipRepeat;
 ...
 * @author Benjamin
 */
public final class MSDClipRepeat{
	
	/**
	 * This will perform an all in one calculation to get mean square displacement information.
	 * @param args {binary trajectory file, working directory, output description file, number of resamples, maximum curvature, minimum signal to noise, dimension, timestep clip}
	...
	public static void main(String[] args) throws IOException{
		...
		if(dstNam == null || srcNam==null || wrkDir==null || numStr==null || dimStr==null || curStr==null || sigStr==null || clpStr==null){
			throw new IllegalArgumentException("Usage: java MSDClipRepeat -src=trajectory -wrk=working_directory -dst=diffusion_summary -num=number_of_resamples -dim=data_dimensionality -cur=maximum_curvature -sig=minimum_signal_to_noise -clp=clip_period [-adc=diffusion_distribution_file]");
		}
		...
		CalculateSquareDisplacementClipRepeat.dataDim = dimension;
		CalculateSquareDisplacementClipRepeat.main(new String[]{"-src="+sourceFileName, "-dst="+workingDirName + File.separatorChar + "SquareDisp.sd.sdb", "-wrk="+workingDirName + File.separatorChar + "working", "-clp="+clpStr});
