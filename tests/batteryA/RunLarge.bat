...
for %%v in (aLResampSort*.dif.sdb) do (
	if exist LargeFull.dif.sdb (
		java -cp ../../DiffResJ.jar diffusion.CollectDiffusionFiles -dst=temp.dif.sdb -src=LargeFull.dif.sdb -src=%%v
		del LargeFull.dif.sdb
		rename temp.dif.sdb LargeFull.dif.sdb
	)
	if not exist LargeFull.dif.sdb copy %%v LargeFull.dif.sdb
)
